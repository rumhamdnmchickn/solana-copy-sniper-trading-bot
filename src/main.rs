...
use solana_vntr_sniper::{
    block_engine::token,
    common::{cache::WALLET_TOKEN_ACCOUNTS, config::Config, constants::RUN_MSG},
    library::{
        blockhash_processor::BlockhashProcessor, cache_maintenance, jupiter_api::JupiterClient,
    },
    processor::{
        sniper_bot::{start_dex_monitoring, start_target_wallet_monitoring, SniperConfig},
        swap::SwapProtocol,
    },
};
...
async fn initialize_token_account_list(config: &Config) {
    let logger = solana_vntr_sniper::common::logger::Logger::new(
        "[INIT-TOKEN-ACCOUNTS] => ".green().to_string(),
    );

    if let Ok(wallet_pubkey) = config.app_state.wallet.try_pubkey() {
        logger.log(format!(
            "Initializing token account list for wallet: {}",
            wallet_pubkey
        ));
        ...
        match accounts {
            Ok(accounts) => {
                ...
                logger.log(format!(
                    "✅ Token account cache initialized with {} accounts",
                    WALLET_TOKEN_ACCOUNTS.size()
                ));
            }
            Err(e) => {
                logger.log(
                    format!("❌ Error fetching token accounts: {}", e)
                        .red()
                        .to_string(),
                );
                logger.log(
                    "⚠️  Cache will be populated as new accounts are discovered"
                        .yellow()
                        .to_string(),
                );
            }
        ...
        logger.log(
            "❌ Failed to get wallet pubkey, can't initialize token account list"
                .red()
                .to_string(),
        );
    }
...
async fn wrap_sol(config: &Config, amount: f64) -> Result<(), String> {
    let logger =
        solana_vntr_sniper::common::logger::Logger::new("[WRAP-SOL] => ".green().to_string());
        ...
        .send_and_confirm_transaction(&transaction)
    {
        Ok(signature) => {
            logger.log(format!(
                "SOL wrapped successfully, signature: {}",
                signature
            ));
            Ok(())
        }
        Err(e) => Err(format!("Failed to wrap SOL: {}", e)),
    }
}
...
async fn unwrap_sol(config: &Config) -> Result<(), String> {
    let logger =
        solana_vntr_sniper::common::logger::Logger::new("[UNWRAP-SOL] => ".green().to_string());
        ...
        .send_and_confirm_transaction(&transaction)
    {
        Ok(signature) => {
            logger.log(format!(
                "WSOL unwrapped successfully, signature: {}",
                signature
            ));
            Ok(())
        }
        Err(e) => Err(format!("Failed to unwrap WSOL: {}", e)),
    }
}
...
async fn sell_all_tokens(config: &Config) -> Result<(), String> {
    let logger = solana_vntr_sniper::common::logger::Logger::new(
        "[SELL-ALL-TOKENS] => ".green().to_string(),
    );
    let quote_logger =
        solana_vntr_sniper::common::logger::Logger::new("[JUPITER-QUOTE] => ".blue().to_string());
    let execute_logger =
        solana_vntr_sniper::common::logger::Logger::new("[EXECUTE-SWAP] => ".yellow().to_string());
    let sell_logger =
        solana_vntr_sniper::common::logger::Logger::new("[SELL-TOKEN] ".cyan().to_string());
    ...
    logger.log(format!(
        "🔍 Scanning wallet {} for tokens to sell",
        wallet_pubkey
    ));
    ...
    for account_info in accounts {
        ...
        let account_data = match config.app_state.rpc_client.get_account(&token_account) {
            ...
            Err(e) => {
                logger.log(
                    format!("Failed to get account data for {}: {}", token_account, e)
                        .red()
                        .to_string(),
                );
                continue;
        ...
        if let Ok(token_data) = spl_token::state::Account::unpack(&account_data.data) {
            ...
            let mint_data = match config.app_state.rpc_client.get_account(&token_data.mint) {
                ...
                Err(e) => {
                    logger.log(
                        format!("Failed to get mint data for {}: {}", token_data.mint, e)
                            .yellow()
                            .to_string(),
                    );
                    continue;
            ...
            let mint_info = match spl_token::state::Mint::unpack(&mint_data.data) {
                ...
                Err(e) => {
                    logger.log(
                        format!("Failed to parse mint data for {}: {}", token_data.mint, e)
                            .yellow()
                            .to_string(),
                    );
                    continue;
    ...
    if tokens_to_sell.is_empty() {
        logger.log(
            "No tokens found to sell (excluding SOL/WSOL)"
                .yellow()
                .to_string(),
        );
        return Ok(());
    ...
    for (mint, amount, decimals) in tokens_to_sell {
        ...
        match jupiter_client.get_quote(&mint, sol_mint, amount, 100).await {
            Ok(quote) => {
                ...
                quote_logger.log(format!(
                    "Raw quote response (first 500 chars): {}",
                ...
                match jupiter_client
                    .sell_token_with_jupiter(&mint, amount, 500, &config.app_state.wallet)
                    .await
                {
                    Ok(signature) => {
                        ...
                        execute_logger
                            .log(format!("Jupiter sell transaction confirmed: {}", signature));

                        // Log the successful sell
                        sell_logger.log(format!(
                            "{} => Token sold successfully! Signature: {}",
                            mint, signature
                        ));
                        ...
                        logger.log(
                            format!("✅ Successfully sold {}: {}", mint, signature)
                                .green()
                                .to_string(),
                        );
                        sold_count += 1;
                    ...
                    Err(e) => {
                        logger.log(
                            format!(
                                "❌ Failed to get sell transaction for token {}: {}",
                                mint, e
                            )
                            .red()
                            .to_string(),
            ...
            Err(e) => {
                logger.log(
                    format!("❌ Failed to get quote for token {}: {}", mint, e)
                        .red()
                        .to_string(),
                );
                failed_count += 1;
    ...
    logger.log(
        format!(
            ...
            sold_count, failed_count, sol_received_display
        )
        .cyan()
        .bold()
        .to_string(),
    ...
    if failed_count > 0 {
        Err(format!(
            "Failed to sell {} out of {} tokens",
            failed_count, total_token_count
        ))
    } else {
        Ok(())
    }
...
async fn close_all_token_accounts(config: &Config) -> Result<(), String> {
    let logger = solana_vntr_sniper::common::logger::Logger::new(
        "[CLOSE-TOKEN-ACCOUNTS] => ".green().to_string(),
    );
    ...
    for account_info in accounts {
        ...
        if let Ok(token_data) = spl_token::state::Account::unpack(&account_data.data) {
            if token_data.mint == spl_token::native_mint::id() && token_data.amount > 0 {
                logger.log(format!(
                    "Skipping WSOL account with non-zero balance: {} ({})",
                    token_account,
        ...
        .map_err(|e| {
            format!(
                "Failed to create close instruction for {}: {}",
                token_account, e
            )
        })?;
            ...
            .send_and_confirm_transaction(&transaction)
        {
            Ok(signature) => {
                logger.log(format!(
                    "Closed token account {}, signature: {}",
            ...
            Err(e) => {
                logger.log(
                    format!("Failed to close token account {}: {}", token_account, e)
                        .red()
                        .to_string(),
                );
                failed_count += 1;
...
async fn main() {
    ...
    let selling_config =
        solana_vntr_sniper::processor::selling_strategy::SellingConfig::set_from_env();
        ...
        .await
    {
        ...
        Err(e) => {
            eprintln!(
                "⚠️  Failed to initialize copy selling for existing tokens: {}",
                e
            );
        }
    }
    ...
    if target_addresses.is_empty() {
        eprintln!(
            "No COPY_TRADING_TARGET_ADDRESS specified. Please set this environment variable."
        );
        return;
    }
