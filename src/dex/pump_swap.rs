...
pub struct PumpSwap {
    ...
    pub rpc_nonblocking_client:
        Option<Arc<anchor_client::solana_client::nonblocking::rpc_client::RpcClient>>,
}
...
impl PumpSwap {
    ...
    pub async fn get_pool_info(
        &self,
        mint_str: &str,
    ) -> Result<(Pubkey, Pubkey, Pubkey, u64, u64)> {
        let mint = Pubkey::from_str(mint_str).map_err(|_| anyhow!("Invalid mint address"))?;
        let rpc_client = self
            .rpc_client
            .clone()
        ...
        let (pool_id, base_reserve, quote_reserve) =
            get_pool_info_for_price(rpc_client, mint).await?;
    ...
    pub async fn build_swap_from_parsed_data(
        ...
        let (base_amount, quote_amount, accounts) = match swap_config.swap_direction {
            SwapDirection::Buy => {
                self.prepare_buy_swap_from_parsed(
                    trade_info,
                    owner,
                    mint,
                    pool_id,
            ...
            SwapDirection::Sell => {
                self.prepare_sell_swap_from_parsed(
                    trade_info,
                    owner,
                    mint,
                    pool_id,
...
fn create_buy_accounts(
    ...
    let coin_creator_vault_ata =
        get_associated_token_address(&coin_creator_vault_authority, &quote_mint);
    ...
    Ok(vec![
        ...
        AccountMeta::new(
            get_associated_token_address(&PUMP_SWAP_FEE_RECIPIENT, &quote_mint),
            false,
        ),
