...
impl HealthCheckManager {
    pub fn new(zeroslot_client: Arc<Client>) -> Self {
        Self {
    ...
    pub async fn start_monitoring(&self) -> Result<()> {
        self.logger.log(
            "Starting health check monitoring for Zeroslot RPC service"
                .green()
                .to_string(),
        );
        ...
        HEALTH_STATUS.insert(
            "zeroslot".to_string(),
            HealthStatus::new("zeroslot".to_string()),
        );
    ...
    async fn start_zeroslot_monitoring(&self) {
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                Self::update_health_status("zeroslot", result.is_ok(), response_time, &logger)
                    .await;

                if let Err(e) = result {
                    logger.log(
                        format!("Zeroslot health check failed: {}", e)
                            .red()
                            .to_string(),
                    );
                } else {
                    logger.log(
                        format!("Zeroslot keepalive successful ({}ms)", response_time)
                            .green()
                            .to_string(),
                    );
                }
    ...
    async fn check_zeroslot_health(client: &Client) -> Result<()> {
        ...
        if response.status().is_success() {
            ...
            Err(anyhow::anyhow!(
                "Zeroslot health check failed with status: {}",
                response.status()
            ))
        }
    }
            ...
            if status.consecutive_failures >= 3 {
                logger.log(
                    format!(
                        ...
                        service_name, status.consecutive_failures
                    )
                    .red()
                    .bold()
                    .to_string(),
    ...
    pub fn get_healthiest_service(landing_mode: &TransactionLandingMode) -> Option<String> {
        match landing_mode {
            TransactionLandingMode::Zeroslot => {
                if Self::is_service_healthy("zeroslot") {
                    Some("zeroslot".to_string())
                } else {
                    None
                }
            }
    ...
    pub fn log_all_health_status(logger: &Logger) {
        logger.log(
            "=== Zeroslot RPC Service Health Status ==="
                .blue()
                .bold()
                .to_string(),
        );
