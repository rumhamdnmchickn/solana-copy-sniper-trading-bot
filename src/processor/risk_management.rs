...
use crate::common::config::{import_env_var, AppState, SwapConfig};
use crate::common::logger::Logger;
use crate::processor::sniper_bot::{BoughtTokenInfo, BOUGHT_TOKEN_LIST};
...
impl RiskManagementService {
    ...
    pub async fn start(&self) -> Result<(), String> {
        ...
        loop {
            ...
            if let Err(e) = self.check_target_balances().await {
                self.logger.log(
                    format!("Error during balance check: {}", e)
                        .red()
                        .to_string(),
                );
            }
    ...
    async fn check_target_balances(&self) -> Result<(), String> {
        self.logger
            .log("🔍 Checking target wallet balances...".cyan().to_string());
        ...
        if held_tokens.is_empty() {
            self.logger.log(
                "No tokens currently held, skipping balance check"
                    .yellow()
                    .to_string(),
            );
            return Ok(());
        ...
        self.logger.log(
            format!("Found {} held tokens to check", held_tokens.len())
                .cyan()
                .to_string(),
        );
        ...
        for target_address in &self.config.target_addresses {
            if let Err(e) = self
                .check_target_wallet_balances(target_address, &held_tokens)
                .await
            {
                self.logger.log(
                    format!(
                        "Error checking balances for target {}: {}",
                        target_address, e
                    )
                    .red()
                    .to_string(),
    ...
    async fn check_target_wallet_balances(
        ...
        let target_balances = self
            .get_target_token_balances(&target_pubkey, held_tokens)
            .await?;
            ...
            if let Some(target_balance) = target_balances.get(token_mint) {
                self.logger.log(
                    format!(
                        ...
                        token_mint, target_balance, self.config.minimum_target_balance
                    )
                    .cyan()
                    .to_string(),
                );
            ...
            if let Ok(account_pubkey) = Pubkey::from_str(&account_info.pubkey) {
                    ...
                    .get_account(&account_pubkey)
                {
                    if let Ok(parsed_account) = TokenAccount::unpack(&account_data.data) {
                        ...
                        if held_tokens
                            .iter()
                            .any(|(token_mint, _)| token_mint == &mint_str)
                        {
                            // Get mint info to calculate actual balance
                            if let Ok(mint_data) = self
                                .config
                                .app_state
                                .rpc_client
                                .get_account(&parsed_account.mint)
                            {
                                if let Ok(mint_info) =
                                    spl_token::state::Mint::unpack(&mint_data.data)
                                {
                                    let raw_balance = parsed_account.amount;
                                    let decimals = mint_info.decimals;
                                    let actual_balance =
                                        raw_balance as f64 / 10_f64.powi(decimals as i32);
