...
lazy_static! {
    ...
    pub static ref TOKEN_TRACKING: Arc<DashMap<String, TokenTrackingInfo>> =
        Arc::new(DashMap::new());
    pub static ref HISTORICAL_TRADES: Arc<DashMap<(), VecDeque<TradeExecutionRecord>>> =
        Arc::new({
            let map = DashMap::new();
...
impl ProfitTakingConfig {
    pub fn set_from_env() -> Self {
        let target_percentage = import_env_var("PROFIT_TAKING_TARGET_PERCENTAGE")
            .parse::<f64>()
            .unwrap_or(1.0);
        let scale_out_percentages =
            parse_vec_f64(import_env_var("PROFIT_TAKING_SCALE_OUT_PERCENTAGES"))
                .into_iter()
    ...
    fn default() -> Self {
        Self {
            ...
            dynamic_thresholds: vec![
                ...
                }, // >20% PnL: 5% trailing stop
                TrailingStopThreshold {
                    pnl_threshold: 50.0,
                    trail_percentage: 10.0,
                }, // >50% PnL: 10% trailing stop
                TrailingStopThreshold {
                    pnl_threshold: 100.0,
                    trail_percentage: 30.0,
                }, // >100% PnL: 30% trailing stop
                TrailingStopThreshold {
                    pnl_threshold: 200.0,
                    trail_percentage: 100.0,
                }, // >200% PnL: 100% trailing stop
                TrailingStopThreshold {
                    pnl_threshold: 500.0,
                    trail_percentage: 100.0,
                }, // >500% PnL: 100% trailing stop
                TrailingStopThreshold {
                    pnl_threshold: 1000.0,
                    trail_percentage: 100.0,
                }, // >1000% PnL: 100% trailing stop
            ],
        }
    }
}
...
impl TrailingStopConfig {
    pub fn set_from_env() -> Self {
        let activation_percentage = import_env_var("TRAILING_STOP_ACTIVATION_PERCENTAGE")
            .parse::<f64>()
            .unwrap_or(20.0);
        let trail_percentage = import_env_var("TRAILING_STOP_TRAIL_PERCENTAGE")
            .parse::<f64>()
            .unwrap_or(10.0);
...
impl LiquidityMonitorConfig {
    pub fn set_from_env() -> Self {
        let min_absolute_liquidity = import_env_var("MIN_ABSOLUTE_LIQUIDITY")
            .parse::<f64>()
            .unwrap_or(1.0);
        let max_acceptable_drop = import_env_var("MAX_ACCEPTABLE_DROP")
            .parse::<f64>()
            .unwrap_or(0.5);
...
impl VolumeAnalysisConfig {
    pub fn set_from_env() -> Self {
        let lookback_period = import_env_var("VOLUME_ANALYSIS_LOOKBACK_PERIOD")
            .parse::<usize>()
            .unwrap_or(20);
        let spike_threshold = import_env_var("VOLUME_ANALYSIS_SPIKE_THRESHOLD")
            .parse::<f64>()
            .unwrap_or(3.0);
        let drop_threshold = import_env_var("VOLUME_ANALYSIS_DROP_THRESHOLD")
            .parse::<f64>()
            .unwrap_or(0.3);
...
impl TimeExitConfig {
    pub fn set_from_env() -> Self {
        let max_hold_time_secs = import_env_var("MAX_HOLD_TIME_SECS")
            .parse::<u64>()
            .unwrap_or(3600);
        let min_profit_time_secs = import_env_var("MIN_PROFIT_TIME_SECS")
            .parse::<u64>()
            .unwrap_or(120);
    ...
    fn default() -> Self {
        Self {
            whale_selling_thresholds: vec![
                WhaleSellingThreshold {
                    pnl_threshold: 10.0,
                    whale_limit_sol: 5.0,
                    use_emergency_zeroslot: true,
                },
                WhaleSellingThreshold {
                    pnl_threshold: 30.0,
                    whale_limit_sol: 6.0,
...
impl DynamicWhaleSelling {
    pub fn set_from_env() -> Self {
        let retracement_percentage = import_env_var("DYNAMIC_RETRACEMENT_PERCENTAGE")
            .parse::<f64>()
            .unwrap_or(15.0);
...
impl SellingConfig {
    pub fn set_from_env() -> Self {
        ...
        let retracement_threshold = import_env_var("RETRACEMENT_THRESHOLD")
            .parse::<f64>()
            .unwrap_or(15.0);
        let min_liquidity = import_env_var("MIN_LIQUIDITY")
            .parse::<f64>()
            .unwrap_or(1.0);
...
impl TokenManager {
    ...
    pub async fn update_token(&self, token_mint: &str, metrics: TokenMetrics) -> Result<()> {
        ...
        self.logger
            .log(format!("Updated metrics for token: {}", token_mint));
        Ok(())
    }
    ...
    pub async fn remove_token(&self, token_mint: &str) -> Result<()> {
        if TOKEN_METRICS.remove(token_mint).is_some() {
            ...
            self.logger
                .log(format!("Removed token from tracking: {}", token_mint));
        } else {
            self.logger
                .log(format!("Token not found for removal: {}", token_mint));
        }
        Ok(())
    }

    /// Comprehensive verification and cleanup for selling strategy tokens
    pub async fn verify_and_cleanup_sold_tokens(&self, app_state: &Arc<AppState>) -> Result<usize> {
        self.logger.log(
            "🔄 Starting comprehensive cleanup for selling strategy tokens..."
                .blue()
                .to_string(),
        );

        // Get all tokens from both tracking systems
        let tokens_to_check: HashSet<String> = TOKEN_METRICS
            .iter()
            .map(|entry| entry.key().clone())
        ...
        if tokens_to_check.is_empty() {
            self.logger.log(
                "📝 No tokens to check in selling strategy cleanup"
                    .blue()
                    .to_string(),
            );
            return Ok(0);
        ...
        self.logger.log(format!(
            "🔍 Checking {} tokens in selling strategy cleanup",
            tokens_to_check.len()
        ));
        ...
        for token_mint in tokens_to_check {
            match self.verify_token_balance(&token_mint, app_state).await {
                ...
                Err(e) => {
                    self.logger.log(
                        format!(
                            "⚠️  Error verifying token balance for {}: {}",
                            token_mint, e
                        )
                        .yellow()
                        .to_string(),
    ...
    pub async fn log_token_portfolio(&self) {
        ...
        if token_count == 0 {
            self.logger
                .log("No tokens currently in portfolio".yellow().to_string());
            return;
        }

        self.logger.log(
            format!("Current portfolio contains {} tokens:", token_count)
                .green()
                .to_string(),
        );
    ...
    pub async fn monitor_all_tokens(&self, engine: &SellingEngine) -> Result<()> {
        let active_tokens: Vec<String> = TOKEN_METRICS
            .iter()
            .map(|entry| entry.key().clone())
            .collect();
        ...
        if active_tokens.is_empty() {
            self.logger
                .log("No active tokens to monitor".yellow().to_string());
            return Ok(());
        }

        self.logger
            .log(format!("Monitoring {} active tokens", active_tokens.len()));
        ...
        for token_mint in active_tokens {
            ...
            match engine.evaluate_sell_conditions(&token_mint).await {
                            ...
                            tokio::spawn(async move {
                                ...
                                match engine_clone.metrics_to_trade_info(&token_mint_clone).await {
                                    Ok(trade_info) => {
                                        ...
                                        let protocol = if let Some(metrics) =
                                            TOKEN_METRICS.get(&token_mint_clone)
                                        {
                                            ...
                                            .await
                                        {
                                            let logger = Logger::new(
                                                "[TOKEN-MANAGER-WHALE] => ".red().to_string(),
                                            );
                                            logger.log(format!(
                                                "Failed to whale emergency sell token {}: {}",
                                                token_mint_clone, e
                                            ));
                                        }
                                    }
                            ...
                            tokio::spawn(async move {
                                ...
                                match engine_clone.metrics_to_trade_info(&token_mint_clone).await {
                                    Ok(trade_info) => {
                                        ...
                                        {
                                            let logger = Logger::new(
                                                "[TOKEN-MANAGER-EMERGENCY] => ".red().to_string(),
                                            );
                                            logger.log(format!(
                                                "Failed to emergency sell token {}: {}",
                                                token_mint_clone, e
                                            ));
                                        }
                                    }
                                    Err(e) => {
                                        ...
                                        logger.log(format!(
                                            "Failed to create trade info for emergency sell {}: {}",
                                            token_mint_clone, e
                                        ));
                                    }
                                }
                ...
                Err(e) => {
                    self.logger.log(
                        format!("Error evaluating sell conditions for {}: {}", token_mint, e)
                            .red()
                            .to_string(),
                    );
                }
...
impl SellingEngine {
    ...
    pub fn log_selling_parameters(&self) {
        self.logger
            .log("📊 SELLING STRATEGY PARAMETERS:".cyan().bold().to_string());
        self.logger.log(
            format!("  🎯 Take Profit: {:.1}%", self.config.take_profit)
                .green()
                .to_string(),
        );
        self.logger.log(
            format!("  🛑 Stop Loss: {:.1}%", self.config.stop_loss)
                .red()
                .to_string(),
        );
        self.logger.log(
            format!(
                "  ⏰ Max Hold Time: {} hour(s)",
                self.config.max_hold_time / 3600
            )
            .blue()
            .to_string(),
        );
        ...
        self.logger.log(
            format!("  💧 Min Liquidity: {:.1} SOL", self.config.min_liquidity)
                .purple()
                .to_string(),
        );
        ...
        for threshold in &self.config.trailing_stop.dynamic_thresholds {
            self.logger.log(
                format!(
                    "    📈 PNL >= {:.0}%: {:.0}% trailing stop",
                    threshold.pnl_threshold, threshold.trail_percentage
                )
                .green()
                .to_string(),
            );
        ...
        self.logger.log(
            format!(
                ...
                self.config.trailing_stop.activation_percentage
            )
            .yellow()
            .to_string(),
        );
        self.logger.log(
            "  🚫 Token Rebuying: DISABLED (permanent blacklist)"
                .red()
                .to_string(),
        );
        self.logger
            .log("  🚀 Copy Selling Method: ZEROSLOT".green().to_string());

        // Log dynamic whale selling thresholds
        self.logger.log(
            "  🐋 DYNAMIC WHALE SELLING THRESHOLDS:"
                .cyan()
                .bold()
                .to_string(),
        );
        for threshold in &self.config.dynamic_whale_selling.whale_selling_thresholds {
            self.logger.log(
                format!(
                    "    💎 PNL >= {:.0}%: Whale Limit {:.1} SOL (Emergency Zeroslot: {})",
                    threshold.pnl_threshold,
                    threshold.whale_limit_sol,
                    if threshold.use_emergency_zeroslot {
                        "✅"
                    } else {
                        "❌"
                    }
    ...
    pub async fn initialize_copy_selling_for_existing_tokens(&self) -> Result<usize> {
        self.logger.log(
            "🔍 Checking for existing token balances to initialize copy selling..."
                .cyan()
                .to_string(),
        );
        ...
        let accounts = self
            ...
            .get_token_accounts_by_owner(
                &wallet_pubkey,
                anchor_client::solana_client::rpc_request::TokenAccountsFilter::ProgramId(
                    token_program,
                ),
            )
        ...
        for account_info in accounts {
            ...
            if let Ok(parsed_account) = spl_token::state::Account::unpack(&account_data.data) {
                ...
                self.logger.log(
                    format!(
                        "📦 Found existing token balance: {} ({:.6} tokens)",
                ...
                let current_price = match self.get_current_price(&token_mint).await {
                    ...
                    Err(_) => {
                        self.logger.log(
                            format!("⚠️  Could not get price for {}, skipping", token_mint)
                                .yellow()
                                .to_string(),
                        );
                        continue;
                ...
                self.logger.log(
                    format!(
                        "✅ Initialized copy selling for existing token: {}",
                        token_mint
                    )
                    .green()
                    .to_string(),
        ...
        if initialized_count > 0 {
            self.logger.log(
                format!(
                    "🎯 Copy selling initialized for {} existing tokens",
                    initialized_count
                )
                .green()
                .bold()
            ...
            self.logger.log(
                "📭 No existing token balances found to initialize for copy selling"
                    .blue()
                    .to_string(),
            );
        }
    ...
    pub async fn update_metrics(
        ...
        let actual_token_balance = match self
            .app_state
            .rpc_nonblocking_client
            .get_token_account(&ata)
            .await
        ...
        if price <= 0.0 {
            logger.log(format!(
                "Invalid price calculated: {} (sol_change: {}, token_change: {})",
        ...
        if is_buy {
            ...
            if token_amount > 0.0 {
                ...
                logger.log(format!(
                    ...
                    old_entry_price,
                    new_entry_price,
                    if entry.amount_held > 0.0 && old_entry_price > 0.0 {
                        "weighted avg"
                    } else {
                        "first purchase"
                    }
                ...
                logger.log(
                    "Warning: Buy transaction detected but token_amount is 0"
                        .yellow()
                        .to_string(),
                );
            }
            ...
            if entry.entry_price == 0.0 && price > 0.0 {
                ...
                logger.log(
                    format!("Set entry price from sell transaction: {}", price)
                        .yellow()
                        .to_string(),
                );
            }
...
pub async fn evaluate_sell_conditions(&self, token_mint: &str) -> Result<(bool, bool)> {
        ...
        if let Some(whale_threshold) = self
            .config
            .dynamic_whale_selling
        ...
        if time_held > self.config.max_hold_time {
            self.logger.log(
                format!(
                    "⏰ Selling due to max hold time exceeded: {}s > {}s (1 hour)",
                    time_held, self.config.max_hold_time
                )
                .yellow()
                .to_string(),
            );
        ...
        if pnl <= self.config.stop_loss {
            self.logger.log(
                format!(
                    "🛑 Selling due to stop loss triggered: {:.2}% <= {:.2}%",
        ...
        if pnl > 0.0 && retracement >= self.config.dynamic_whale_selling.retracement_percentage {
            self.logger.log(
                format!(
        ...
        if pnl >= self.config.take_profit {
            self.logger.log(
                format!(
                    "🎯 Selling due to take profit reached: {:.2}% >= {:.2}%",
        ...
        if metrics.liquidity_at_current > 0.0 && metrics.liquidity_at_entry > 0.0 {
            let liquidity_drop = (metrics.liquidity_at_entry - metrics.liquidity_at_current)
                / metrics.liquidity_at_entry
                * 100.0;
            ...
            if metrics.liquidity_at_current < self.config.liquidity_monitor.min_absolute_liquidity {
                self.logger.log(
                    format!(
                        "💧 Selling due to low absolute liquidity: {:.2} SOL < {:.2} SOL",
                        metrics.liquidity_at_current,
                        self.config.liquidity_monitor.min_absolute_liquidity
                    )
                    .red()
                    .to_string(),
                );
            ...
            if liquidity_drop >= self.config.liquidity_monitor.max_acceptable_drop * 100.0 {
                self.logger.log(
                    format!(
                        "💧 Selling due to liquidity drop: {:.2}% >= {:.2}%",
                        liquidity_drop,
                        self.config.liquidity_monitor.max_acceptable_drop * 100.0
                    )
                    .red()
                    .to_string(),
                );
    ...
    async fn get_current_price(&self, token_mint: &str) -> Result<f64> {
        ...
        match protocol {
            ...
            SwapProtocol::Auto | SwapProtocol::Unknown => {
                self.logger.log(
                    "Auto/Unknown protocol in get_current_price, using cached metrics"
                        .yellow()
                        .to_string(),
                );
        ...
        if virtual_token != 0 {
            ...
            if let Some(current_price) = self.calculate_current_price(trade_info) {
                ...
                if price_diff > 0.1 {
                    // 10% difference
                    return Some(format!(
                        "Possible price manipulation: {:.2}% difference",
                        price_diff * 100.0
                    ));
                }
            }
    ...
    pub fn check_large_holder_actions(&self, trade_info: &TradeInfoFromToken) -> Option<String> {
        ...
        if trade_size > liquidity * 0.1 {
            // 10% of liquidity
            return Some(format!(
                "Large trade size detected: {:.2} SOL ({:.2}% of liquidity)",
                trade_size,
                (trade_size / liquidity) * 100.0
            ));
        }
    ...
    pub fn adjust_strategy_based_on_market(&mut self, market_condition: MarketCondition) {
        self.logger.log(format!(
            "Adjusting strategy for market condition: {:?}",
            market_condition
        ));
        ...
        match market_condition {
            MarketCondition::Bullish => {
                ...
                self.logger.log(
                    "Adjusted for bullish market: increased profit targets"
                        .green()
                        .to_string(),
                );
            }
            MarketCondition::Bearish => {
                ...
                self.logger.log(
                    "Adjusted for bearish market: reduced profit targets"
                        .yellow()
                        .to_string(),
                );
            }
            MarketCondition::Volatile => {
                ...
                self.logger.log(
                    "Adjusted for volatile market: tightened stops"
                        .yellow()
                        .to_string(),
                );
            }
            MarketCondition::Stable => {
                ...
                self.logger.log(
                    "Adjusted for stable market: letting winners run longer"
                        .green()
                        .to_string(),
                );
            }
    ...
    pub async fn metrics_to_trade_info(&self, token_mint: &str) -> Result<TradeInfoFromToken> {
        ...
        let dex_type = match protocol_to_use {
            ...
            SwapProtocol::Auto => {
                ...
                self.logger.log(
                    "Auto protocol detected, defaulting to PumpFun"
                        .yellow()
                        .to_string(),
                );
                DexType::PumpFun
            ...
            SwapProtocol::Unknown => {
                self.logger.log(
                    "Unknown protocol detected, defaulting to PumpFun"
                        .yellow()
                        .to_string(),
                );
                DexType::PumpFun
        ...
        let (
            ...
            SwapProtocol::PumpSwap => {
                ...
                match pump_swap.get_pool_info(token_mint).await {
                    Ok((pool_id, base_mint, quote_mint, base_reserve, quote_reserve)) => {
                        ...
                        let est_sol_amount =
                            (metrics.current_price * token_amount * 1_000_000_000.0) as u64;
                    ...
                    Err(e) => {
                        self.logger
                            .log(format!("Failed to get pool info: {}", e).red().to_string());
            ...
            SwapProtocol::PumpFun => {
                ...
                let virtual_sol_reserves =
                    (virtual_token_reserves as f64 * metrics.current_price) as u64;
            ...
            SwapProtocol::Auto | SwapProtocol::Unknown => {
                ...
                self.logger.log(
                    "Using PumpFun defaults for Auto/Unknown protocol"
                        .yellow()
                        .to_string(),
                );
    ...
    pub async fn analyze_market_condition(
        &self,
        recent_trades: &[TradeInfoFromToken],
    ) -> MarketCondition {
        ...
        let mut price_time_pairs: Vec<(u64, f64)> = timestamps
            .iter()
            .zip(prices.iter())
            .map(|(t, p)| (*t, *p))
            .collect();
        ...
        let sorted_prices: Vec<f64> = price_time_pairs.iter().map(|(_, p)| *p).collect();
        ...
        let price_volatility = if !sorted_prices.is_empty() {
            ...
            let variance = sorted_prices
                .iter()
                .map(|p| (p - mean_price).powi(2))
                .sum::<f64>()
                / sorted_prices.len() as f64;
        ...
        if price_volatility > 0.15 {
            ...
            if price_trend > 0.05 {
                self.logger.log(
                    "Market condition: Bullish with high volatility"
                        .green()
                        .to_string(),
                );
                MarketCondition::Bullish
                ...
                self.logger.log(
                    "Market condition: Bearish with high volatility"
                        .red()
                        .to_string(),
                );
                MarketCondition::Bearish
                ...
                self.logger.log(
                    "Market condition: Volatile with no clear trend"
                        .yellow()
                        .to_string(),
                );
                MarketCondition::Volatile
            ...
            if price_trend > 0.05 {
                self.logger
                    .log("Market condition: Stable uptrend".green().to_string());
                MarketCondition::Bullish
            } else if price_trend < -0.05 {
                self.logger
                    .log("Market condition: Stable downtrend".red().to_string());
                MarketCondition::Bearish
            } else {
                self.logger
                    .log("Market condition: Stable sideways".blue().to_string());
                MarketCondition::Stable
            }
        }
    }
    ...
    pub async fn unified_emergency_sell(
        &self,
        token_mint: &str,
        is_whale_emergency: bool,
        parsed_data: Option<&TradeInfoFromToken>,
        protocol: Option<SwapProtocol>,
    ) -> Result<String> {
        ...
        let result = timeout(
            timeout_duration,
            self.execute_emergency_sell_internal(
                token_mint,
                is_whale_emergency,
                parsed_data,
                protocol,
        ...
        match result {
            ...
            Err(_timeout_err) => {
                self.logger.log(
                    format!(
                        "🚨 Emergency sell timed out for token: {} ({}s timeout)",
                        token_mint,
                ...
                Err(anyhow!(
                    "Emergency sell operation timed out after {}s",
                    timeout_duration.as_secs()
                ))
            }
        }
    ...
    async fn execute_emergency_sell_internal(
        &self,
        token_mint: &str,
        is_whale_emergency: bool,
        parsed_data: Option<&TradeInfoFromToken>,
        ...
        if is_whale_emergency {
            self.logger.log(
                format!(
                    "🐋 WHALE EMERGENCY SELL triggered for token: {}",
                    token_mint
                )
                .red()
                .bold()
            ...
            self.logger.log(
                format!(
                    "🚨 REGULAR EMERGENCY SELL triggered for token: {}",
                    token_mint
                )
                .red()
                .bold()
            ...
            .await
        {
            Ok(Some(account)) => {
                ...
                let decimal_amount =
                    amount_value / 10f64.powi(account.token_amount.decimals as i32);
                self.logger.log(
                    format!("Emergency selling {} tokens", decimal_amount)
                        .red()
                        .to_string(),
                );
                decimal_amount
        ...
        let emergency_trade_info = if let Some(data) = parsed_data {
            ...
            TradeInfoFromToken {
                ...
                slot: data.slot,
                signature: if is_whale_emergency {
                    "whale_emergency_sell"
                } else {
                    "regular_emergency_sell"
                }
                .to_string(),
            ...
            match self.metrics_to_trade_info(token_mint).await {
                Ok(trade_info) => {
                    ...
                    emergency_trade_info.signature = if is_whale_emergency {
                        "whale_emergency_sell"
                    } else {
                        "metrics_emergency_sell"
                    }
                    .to_string();
        ...
        let result = match sell_protocol {
            SwapProtocol::PumpFun => {
                self.logger.log(
                    "Using PumpFun protocol for emergency sell"
                        .red()
                        .to_string(),
                );
                ...
                match pump
                    .build_swap_from_parsed_data(&emergency_trade_info, emergency_config)
                    .await
                {
                    Ok((keypair, instructions, price)) => {
                        ...
                        self.logger.log(format!(
                            "Generated emergency PumpFun sell instruction at price: {}",
                            price
                        ));
                        ...
                        .await
                        {
                            Ok(signatures) => {
                                ...
                                self.logger.log(
                                    format!(
                                        "Emergency PumpFun sell transaction sent: {}",
                                        signature
                                    )
                                    .green()
                                    .to_string(),
                            ...
                            Err(e) => {
                                self.logger.log(
                                    format!("Emergency sell transaction failed: {}", e)
                                        .red()
                                        .to_string(),
                                );
                    ...
                    Err(e) => {
                        self.logger.log(
                            format!("Failed to build emergency PumpFun sell instruction: {}", e)
                                .red()
                                .to_string(),
                        );
            ...
            SwapProtocol::PumpSwap => {
                self.logger.log(
                    "Using PumpSwap protocol for emergency sell"
                        .red()
                        .to_string(),
                );
                    ...
                    .await
                {
                    Ok((keypair, instructions, price)) => {
                        ...
                        self.logger.log(format!(
                            "Generated emergency PumpSwap sell instruction at price: {}",
                            price
                        ));
                        // Execute with zeroslot for copy selling
                        match crate::block_engine::tx::new_signed_and_send_zeroslot(
                        ...
                        .await
                        {
                            Ok(signatures) => {
                                ...
                                self.logger.log(
                                    format!(
                                        "Emergency PumpSwap sell transaction sent: {}",
                                        signature
                                    )
                                    .green()
                                    .to_string(),
                    ...
                    Err(e) => {
                        self.logger.log(
                            format!("Failed to build emergency PumpSwap sell instruction: {}", e)
                                .red()
                                .to_string(),
                        );
                        Err(anyhow!("Failed to build emergency sell instruction: {}", e))
            ...
            SwapProtocol::RaydiumLaunchpad => {
                self.logger.log(
                    "Using Raydium protocol for emergency sell"
                        .red()
                        .to_string(),
                );
                    ...
                    .await
                {
                    Ok((keypair, instructions, price)) => {
                        ...
                        self.logger.log(format!(
                            "Generated emergency Raydium sell instruction at price: {}",
                            price
                        ));
                        // Execute with zeroslot for copy selling
                        match crate::block_engine::tx::new_signed_and_send_zeroslot(
                        ...
                        .await
                        {
                            Ok(signatures) => {
                                ...
                                self.logger.log(
                                    format!(
                                        "Emergency Raydium sell transaction sent: {}",
                                        signature
                                    )
                                    .green()
                                    .to_string(),
                    ...
                    Err(e) => {
                        self.logger.log(
                            format!("Failed to build emergency Raydium sell instruction: {}", e)
                                .red()
                                .to_string(),
                        );
                        Err(anyhow!("Failed to build emergency sell instruction: {}", e))
            ...
            SwapProtocol::Auto | SwapProtocol::Unknown => {
                self.logger.log(
                    "Auto/Unknown protocol detected, defaulting to PumpFun for emergency sell"
                        .yellow()
                        .to_string(),
                );
        ...
        let final_result = match result {
            Ok(signature) => {
                self.logger.log(
                    format!("✅ DEX emergency sell successful: {}", signature)
                        .green()
                        .to_string(),
                );
                Ok(signature)
            ...
            Err(dex_error) => {
                self.logger.log(
                    format!(
                        "❌ DEX emergency sell failed: {}. Attempting Jupiter API fallback...",
                        dex_error
                    )
                    .yellow()
                    .to_string(),
                ...
                match self
                    .try_jupiter_fallback_sell(token_mint, token_amount)
                    .await
                {
                    Ok(jupiter_signature) => {
                        self.logger.log(
                            format!(
                                "✅ Jupiter API fallback sell successful: {}",
                                jupiter_signature
                            )
                            .green()
                            .to_string(),
                    ...
                    Err(jupiter_error) => {
                        ...
                        Err(anyhow!(
                            "Both DEX and Jupiter selling failed. DEX error: {}. Jupiter error: {}",
                            dex_error,
                            jupiter_error
                        ))
                    }
        ...
        if final_result.is_ok() {
            ...
            {
                self.logger.log(
                    format!("Failed to record emergency trade execution: {}", e)
                        .red()
                        .to_string(),
                );
            }
    ...
    async fn try_jupiter_fallback_sell(
        &self,
        token_mint: &str,
        token_amount: f64,
    ) -> Result<String> {
        self.logger.log(
            format!(
                "🌌 Attempting Jupiter API fallback sell for {} tokens of {}",
                token_amount, token_mint
            )
            .cyan()
            .to_string(),
        );
        ...
        let raw_token_amount = match self
            .app_state
            .rpc_nonblocking_client
            .get_token_account(&ata)
            .await
        ...
        self.logger.log(
            format!(
                "Jupiter selling {} raw tokens with {}bps slippage",
                raw_token_amount, slippage_bps
            )
            .cyan()
            .to_string(),
        );
            ...
            .await
        {
            Ok(signature) => {
                self.logger.log(
                    format!("🌌 Jupiter API sell completed: {}", signature)
                        .green()
                        .to_string(),
                );
                Ok(signature)
            ...
            Err(e) => {
                self.logger.log(
                    format!("🌌 Jupiter API sell failed: {}", e)
                        .red()
                        .to_string(),
                );
        ...
        match trade_info.dex_type {
            ...
            DexType::PumpFun | _ => {
                ...
                if virtual_token != 0 {
                    ...
                    let scaled_price =
                        ((virtual_sol as f64) * 1_000_000_000.0) / (virtual_token as f64);
    ...
    // Add missing methods
    pub async fn check_liquidity_conditions(
        &self,
        trade_info: &TradeInfoFromToken,
    ) -> Option<String> {
        let liquidity = self.calculate_liquidity(trade_info)?;
    ...
    pub async fn check_price_conditions(&self, trade_info: &TradeInfoFromToken) -> Option<String> {
        ...
        let metrics = self
            .token_manager
            .get_token_metrics(&trade_info.mint)
            .await?;
        ...
        let dynamic_trail_percentage = self
            .config
            .trailing_stop
            .get_trailing_stop_for_pnl(current_pnl);
        ...
        if current_pnl >= self.config.trailing_stop.activation_percentage {
            if retracement >= dynamic_trail_percentage {
                ...
                return Some(format!(
                    "Dynamic trailing stop: {:.2}% retracement (trail: {:.2}%)",
    ...
    pub async fn send_priority_transaction(
            ...
            .await
        {
            ...
            Err(e) => Err(anyhow::anyhow!(
                "Failed to send priority transaction: {}",
                e
            )),
        }
    }
