...
use crate::processor::selling_strategy::{SellingConfig, SellingEngine};
use anyhow::Result;
use bs58;
...
use crate::common::{
    ...
    config::{AppState, Config, SwapConfig},
...
use crate::processor::selling_strategy::{
    TokenMetrics, TokenTrackingInfo as SellingTokenTrackingInfo,
};
...
impl BoughtTokenInfo {
    ...
    pub fn update_price(&mut self, new_price: u64) {
        ...
        if self.pnl_percentage.abs() > 1000.0 {
            // Log if PnL is unusually high
            println!(
                "DEBUG PRICE: Token {} - Entry: {}, Current: {}, PnL: {:.2}%",
    ...
    pub fn should_sell_due_to_trailing_stop(&self) -> bool {
        ...
        let drop_from_highest = ((self.highest_price as f64 - self.current_price as f64)
            / self.highest_price as f64)
            * 100.0;
    ...
    pub fn get_selling_action(&self) -> SellingAction {
        ...
        let max_hold_time = import_env_var("MAX_HOLD_TIME")
            .parse::<u64>()
            .unwrap_or(86400);
        ...
        if self.pnl_percentage >= take_profit {
            return SellingAction::SellAll(format!(
                "Take profit triggered: {:.2}% profit",
                self.pnl_percentage
            ));
        }
    ...
    fn should_sell_due_to_trailing_stop_with_percentage(
        &self,
...
async fn send_heartbeat_ping(
    subscribe_tx: &Arc<
        tokio::sync::Mutex<impl Sink<SubscribeRequest, Error = impl std::fmt::Debug> + Unpin>,
    >,
) -> Result<(), String> {
...
async fn cancel_token_monitoring(token_mint: &str, logger: &Logger) -> Result<(), String> {
    logger.log(format!(
        "🔌 Cancelling monitoring and closing gRPC stream for sold token: {}",
        token_mint
    ));
    ...
    if let Some((_removed_key, (_token_name, cancel_token))) = MONITORING_TASKS.remove(token_mint) {
        ...
        logger.log(format!(
            "✅ Cancelled monitoring task and triggered gRPC stream closure for token: {}",
            token_mint
        ));
    } else {
        logger.log(
            format!(
                "⚠️ No monitoring task found for token: {} (stream may already be closed)",
                token_mint
            )
            .yellow()
            .to_string(),
    ...
    while SHOULD_CONTINUE_STREAMING.load(Ordering::SeqCst) {
        match stream.next().await {
            Some(msg_result) => {
                match msg_result {
                    Ok(msg) => {
                        if let Err(e) = process_message_for_target_monitoring(
                            &msg,
                            &subscribe_tx,
                            config.clone(),
                            &logger,
                        ...
                        {
                            logger
                                .log(format!("Error processing message: {}", e).red().to_string());
                        }
                    }
                    Err(e) => {
    ...
    while SHOULD_CONTINUE_STREAMING.load(Ordering::SeqCst) {
        match stream.next().await {
            Some(msg_result) => {
                match msg_result {
                    Ok(msg) => {
                        if let Err(e) = process_message_for_dex_monitoring(
                            &msg,
                            &subscribe_tx,
                            config.clone(),
                            &logger,
...
async fn verify_transaction(
    ...
    for retry in 0..max_retries {
            ...
            .await
        {
            ...
            Err(e) => {
                logger.log(
                    format!("Failed to get transaction status: {}, retrying...", e)
                        .red()
                        .to_string(),
                );
            }
...
pub async fn execute_buy(
    ...
    if BOUGHT_TOKENS_BLACKLIST.contains_key(&trade_info.mint) {
        logger.log(
            format!(
                "🚫 Token {} is blacklisted (previously bought), skipping buy",
                trade_info.mint
            )
            .yellow()
            .to_string(),
    ...
    let result = match protocol {
        SwapProtocol::PumpFun => {
            ...
            match pump
                .build_swap_from_parsed_data(&trade_info, buy_config.clone())
                .await
            {
                Ok((keypair, instructions, price)) => {
                    logger.log(format!(
                        "Generated PumpFun buy instruction at price: {}",
                        price
                    ));
                    ...
                    println!(
                        "time taken for get_latest_blockhash: {:?}",
                        start_time.elapsed()
                    );
                        ...
                        Ok(signatures) => {
                            ...
                            .await
                            {
                                Ok(verified) => {
                                    if verified {
                                        logger.log(
                                            "Buy transaction verified successfully".to_string(),
                                        );
                                        ...
                                        if let Ok(wallet_pubkey) = app_state.wallet.try_pubkey() {
                                            ...
                                            logger.log(format!(
                                                "Added token account {} to global list",
                                                token_ata
                                            ));
                                            ...
                                            BOUGHT_TOKEN_LIST
                                                .insert(trade_info.mint.clone(), bought_token_info);
                                            logger.log(format!(
                                                "Added {} to enhanced tracking system (PumpFun)",
                                                trade_info.mint
                                            ));
                                            ...
                                            let timestamp = std::time::SystemTime::now()
                                                ...
                                                .unwrap_or_default()
                                                .as_secs();
                                            BOUGHT_TOKENS_BLACKLIST
                                                .insert(trade_info.mint.clone(), timestamp);
                                            logger.log(format!(
                                            ...
                                            if let Err(e) = selling_engine
                                                .update_metrics(&trade_info.mint, &trade_info)
                                                .await
                                            {
                                            ...
                                            logger.log(
                                                format!(
                                                    "✅ Buy transaction completed for token: {}",
                                                    trade_info.mint
                                                )
                                                .green()
                                                .to_string(),
                                ...
                                Err(e) => Err(format!("Transaction verification error: {}", e)),
                            }
                        }
                        Err(e) => Err(format!("Transaction error: {}", e)),
                    }
                }
                Err(e) => Err(format!("Failed to build PumpFun buy instruction: {}", e)),
            }
        }
        SwapProtocol::PumpSwap => {
                ...
                .await
            {
                Ok((keypair, instructions, price)) => {
                    logger.log(format!(
                        "Generated PumpSwap buy instruction at price: {}",
                        price
                    ));
                    logger.log(format!("copy transaction {}", trade_info.signature));
                                    ...
                                    if verified {
                                        ...
                                        if let Ok(wallet_pubkey) = app_state.wallet.try_pubkey() {
                                            ...
                                            BOUGHT_TOKEN_LIST
                                                .insert(trade_info.mint.clone(), bought_token_info);
                                            logger.log(format!(
                                                "Added {} to enhanced tracking system (PumpSwap)",
                                                trade_info.mint
                                            ));
                ...
                Err(e) => Err(format!("Failed to build PumpSwap buy instruction: {}", e)),
            }
        }
        SwapProtocol::RaydiumLaunchpad => {
                ...
                .await
            {
                                    ...
                                    if verified {
                                        ...
                                        if let Ok(wallet_pubkey) = app_state.wallet.try_pubkey() {
                                            ...
                                            BOUGHT_TOKEN_LIST
                                                .insert(trade_info.mint.clone(), bought_token_info);
                                            logger.log(format!(
                                                "Added {} to enhanced tracking system (Raydium)",
                                                trade_info.mint
                                            ));
                ...
                Err(e) => Err(format!(
                    "Failed to build RaydiumLaunchpad buy instruction: {}",
                    e
                )),
            }
        }
        SwapProtocol::Auto | SwapProtocol::Unknown => {
            logger.log(
                "Auto/Unknown protocol detected, defaulting to PumpFun for buy"
                    .yellow()
                    .to_string(),
            );
                                    ...
                                    if verified {
                                        ...
                                        if let Ok(wallet_pubkey) = app_state.wallet.try_pubkey() {
                                            ...
                                            BOUGHT_TOKEN_LIST
                                                .insert(trade_info.mint.clone(), bought_token_info);
                                            logger.log(format!(
                                                "Added {} to bought tokens tracking",
                                                trade_info.mint
                                            ));
                                            ...
                                            tokio::spawn(async move {
                                                start_enhanced_selling_monitor(
                                                    app_state_clone,
                                                    swap_config_clone,
                                                )
                                                .await;
    ...
    if result.is_ok() {
        ...
        println!(
            "DEBUG TRACKING: Adding token {} to BOUGHT_TOKEN_LIST with entry_price: {}",
        ...
        if bought_token_info.entry_price > 0 {
            ...
            println!(
                "WARNING: Refusing to track token {} with entry_price = 0",
                trade_info.mint
            );
        }
        ...
        let _active_tokens: Vec<String> = TOKEN_TRACKING
            .iter()
        ...
        match stream.next().await {
            Some(msg_result) => {
                match msg_result {
                                    ...
                                    if is_sell {
                                        ...
                                        for token_balance in &meta.post_token_balances {
                                                ...
                                                == 0.0
                                            {
                                                ...
                                                crate::processor::selling_strategy::TOKEN_METRICS
                                                    .remove(&token_balance.mint);
                                                crate::processor::selling_strategy::TOKEN_TRACKING
                                                    .remove(&token_balance.mint);
                    ...
                    Err(e) => {
                        logger.log(
                            format!("Wallet monitor stream error: {:?}", e)
                                .red()
                                .to_string(),
                        );
                        // Check if it's a connection limit error
                        if format!("{:?}", e).contains("Maximum connection count reached") {
                            logger.log(
                                "🚫 Wallet monitor connection limit reached - closing stream"
                                    .red()
                                    .bold()
                                    .to_string(),
                            );
...
async fn execute_pumpfun_emergency_sell_with_zeroslot(
    ...
    match pump
        .build_swap_from_parsed_data(trade_info, sell_config)
        .await
    {
        Ok((keypair, instructions, price)) => {
            logger.log(format!(
                "🐋 Generated PumpFun whale emergency sell instruction at price: {}",
                price
            ));

            let recent_blockhash =
                match crate::library::blockhash_processor::BlockhashProcessor::get_latest_blockhash(
                )
                .await
                {
                    Some(hash) => hash,
            ...
            .await
            {
                Ok(signatures) => {
                    ...
                    let signature = &signatures[0];
                    logger.log(format!(
                        "🐋 ZEROSLOT whale emergency sell transaction sent: {}",
                        signature
                    ));

                    verify_transaction(&signature.to_string(), app_state.clone(), logger)
                        .await
        ...
        Err(e) => Err(format!(
            "Failed to build PumpFun whale emergency sell instruction: {}",
            e
        )),
    }
}
...
async fn execute_pumpswap_emergency_sell_with_zeroslot(
        ...
        .await
    {
        Ok((keypair, instructions, price)) => {
            logger.log(format!(
                "🐋 Generated PumpSwap whale emergency sell instruction at price: {}",
                price
            ));
        ...
        Err(e) => Err(format!(
            "Failed to build PumpSwap whale emergency sell instruction: {}",
            e
        )),
    }
}
...
async fn execute_jupiter_emergency_sell(
    ...
    logger.log(
        format!(
            "🪐 Attempting Jupiter API emergency sell for {}",
            token_mint
        )
        .cyan()
        .to_string(),
    ...
    let jupiter_client =
        crate::library::jupiter_api::JupiterClient::new(app_state.rpc_nonblocking_client.clone());
    ...
    let token_balance = match app_state
        .rpc_nonblocking_client
        .get_token_account(&token_account)
        .await
    {
        Ok(Some(account)) => {
            let amount_value = account
                .token_amount
                .amount
                .parse::<u64>()
        ...
        .await
    {
        Ok(signature) => {
            logger.log(
                format!("🪐 Jupiter emergency sell successful: {}", signature)
                    .green()
                    .to_string(),
            );
            Ok(())
        }
        Err(e) => Err(format!("Jupiter emergency sell failed: {}", e)),
    }
}
...
async fn execute_emergency_sell_with_method(
    ...
    match protocol {
        SwapProtocol::PumpFun => {
            if method == "zeroslot" {
                execute_pumpfun_emergency_sell_with_zeroslot(
                    trade_info,
                    sell_config,
                    app_state,
                    logger,
                ...
                execute_pumpfun_emergency_sell_with_normal(
                    trade_info,
                    sell_config,
                    app_state,
                    logger,
        ...
        SwapProtocol::PumpSwap => {
            if method == "zeroslot" {
                execute_pumpswap_emergency_sell_with_zeroslot(
                    trade_info,
                    sell_config,
                    app_state,
                    logger,
                ...
                execute_pumpswap_emergency_sell_with_normal(
                    trade_info,
                    sell_config,
                    app_state,
                    logger,
        ...
        SwapProtocol::RaydiumLaunchpad => {
            if method == "zeroslot" {
                execute_raydium_emergency_sell_with_zeroslot(
                    trade_info,
                    sell_config,
                    app_state,
                    logger,
                ...
                execute_raydium_emergency_sell_with_normal(
                    trade_info,
                    sell_config,
                    app_state,
                    logger,
        ...
        SwapProtocol::Auto | SwapProtocol::Unknown => {
            logger.log(
                "Auto/Unknown protocol, defaulting to PumpFun for emergency sell"
                    .yellow()
                    .to_string(),
            );
            if method == "zeroslot" {
...
async fn execute_pumpfun_emergency_sell_with_normal(
        ...
        .await
    {
        Ok((keypair, instructions, price)) => {
            logger.log(format!(
                "🐋 Generated PumpFun emergency sell (normal RPC) at price: {}",
                price
            ));
            ...
            .await
            {
                Ok(signatures) => {
                    ...
                    let signature = &signatures[0];
                    logger.log(format!(
                        "🐋 NORMAL RPC emergency sell transaction sent: {}",
                        signature
                    ));
                    Ok(())
                }
        ...
        Err(e) => Err(format!(
            "Failed to build PumpFun emergency sell instruction: {}",
            e
        )),
    }
}
...
async fn execute_pumpswap_emergency_sell_with_normal(
        ...
        .await
    {
        Ok((keypair, instructions, price)) => {
            logger.log(format!(
                "🐋 Generated PumpSwap emergency sell (normal RPC) at price: {}",
                price
            ));
        ...
        Err(e) => Err(format!(
            "Failed to build PumpSwap emergency sell instruction: {}",
            e
        )),
    }
}
...
async fn execute_raydium_emergency_sell_with_normal(
        ...
        .await
    {
        Ok((keypair, instructions, price)) => {
            logger.log(format!(
                "🐋 Generated Raydium emergency sell (normal RPC) at price: {}",
                price
            ));
        ...
        Err(e) => Err(format!(
            "Failed to build Raydium emergency sell instruction: {}",
            e
        )),
    }
}
...
pub async fn execute_enhanced_sell(
    ...
    if token_info.entry_price == 0 {
        logger.log(
            format!(
                "WARNING: Token {} has entry_price = 0, buy transaction may not be processed yet",
                token_mint
            )
            .yellow()
            .to_string(),
...
async fn execute_sell_all_enhanced(
    ...
    let token_amount = match app_state
        .rpc_nonblocking_client
    ...
    let trade_info =
        create_sell_trade_info_from_original(token_mint, token_amount, &token_info.trade_info);

    let result = match token_info.protocol {
        SwapProtocol::PumpFun => {
            execute_pumpfun_sell_with_zeroslot(&trade_info, sell_config, app_state.clone(), &logger)
                .await
        }
        SwapProtocol::PumpSwap => {
            execute_pumpswap_sell_with_zeroslot(
                &trade_info,
                sell_config,
                app_state.clone(),
                &logger,
        ...
        SwapProtocol::RaydiumLaunchpad => {
            execute_raydium_sell_with_zeroslot(&trade_info, sell_config, app_state.clone(), &logger)
                .await
        }
        SwapProtocol::Auto | SwapProtocol::Unknown => {
            logger.log(
                "Auto/Unknown protocol detected, defaulting to PumpFun for sell all"
                    .yellow()
                    .to_string(),
            );
            execute_pumpfun_sell_with_zeroslot(
                &trade_info,
                (*swap_config).clone(),
                app_state.clone(),
                &logger,
    ...
    if result.is_ok() {
        ...
        .await
        {
            Ok(cleaned_up) => {
                if cleaned_up {
                    logger.log(format!(
                        "✅ Comprehensive cleanup completed for sell all: {}",
                        token_mint
                    ));
                } else {
                    logger.log(
                        format!(
                            "⚠️  Sell all cleanup verification failed for: {}",
                            token_mint
                        )
                        .yellow()
                        .to_string(),
            ...
            Err(e) => {
                logger.log(
                    format!("❌ Error during sell all cleanup verification: {}", e)
                        .red()
                        .to_string(),
                );
                ...
                logger.log(format!(
                    "Fallback: Removed {} from basic tracking systems",
                    token_mint
                ));
...
async fn cleanup_token_tracking(app_state: &Arc<AppState>) {
    ...
    let tokens_to_check: Vec<String> = BOUGHT_TOKEN_LIST
        .iter()
        .map(|entry| entry.key().clone())
        .collect();
                    ...
                    Ok(account_result) => {
                        match account_result {
                            Some(account) => {
                                ...
                                {
                                    let decimal_amount = amount_value
...
async fn start_enhanced_selling_monitor(app_state: Arc<AppState>, _swap_config: Arc<SwapConfig>) {
...
async fn update_token_price_from_trade_info(
    ...
    if let Some(mut token_info) = BOUGHT_TOKEN_LIST.get_mut(token_mint) {
        ...
        println!(
            "DEBUG PRICE UPDATE FROM TRANSACTION: Token {} - Protocol: {:?}, Old: {}, New: {}",
...
async fn execute_pumpfun_sell_with_zeroslot(
        ...
        .await
    {
        Ok((keypair, instructions, price)) => {
            logger.log(format!(
                "Generated PumpFun sell instruction at price: {}",
                price
            ));
...
async fn execute_pumpswap_sell_with_zeroslot(
        ...
        .await
    {
        Ok((keypair, instructions, price)) => {
            logger.log(format!(
                "Generated PumpSwap sell instruction at price: {}",
                price
            ));
...
async fn execute_raydium_emergency_sell_with_zeroslot(
        ...
        .await
    {
        Ok((keypair, instructions, price)) => {
            logger.log(format!(
                "🐋 Generated Raydium whale emergency sell instruction at price: {}",
                price
            ));
            ...
            .await
            {
                Ok(signatures) => {
                    ...
                    let signature = &signatures[0];
                    logger.log(format!(
                        "🐋 ZEROSLOT Raydium whale emergency sell transaction sent: {}",
                        signature
                    ));
        ...
        Err(e) => Err(format!(
            "Failed to build Raydium whale emergency sell instruction: {}",
            e
        )),
    }
}
...
async fn execute_raydium_sell_with_zeroslot(
        ...
        .await
    {
        Ok((keypair, instructions, price)) => {
            logger.log(format!(
                "Generated Raydium sell instruction at price: {}",
                price
            ));
            ...
            .await
            {
                Ok(signatures) => {
                    ...
                    let signature = &signatures[0];
                    logger.log(format!(
                        "ZEROSLOT Raydium sell transaction sent: {}",
                        signature
                    ));
        ...
        Ok((keypair, instructions, price)) => {
            ...
            .await
            {
                Ok(signatures) => {
                    ...
                    let signature = &signatures[0];
                    logger.log(format!(
                        "NORMAL Raydium sell transaction sent: {}",
                        signature
                    ));
...
pub async fn execute_sell(
        ...
        .await
    {
        Ok(Some(account)) => {
            ...
            logger.log(format!(
                "Token amount to sell (100% of balance): {}",
                decimal_amount
            ));
            decimal_amount
        }
    ...
    if false {
        ...
        logger.log(format!(
            "Executing progressive sell in {} chunks with {} ms intervals",
            chunks_count, interval
        ));
        ...
        for i in 0..chunks_count {
                    ...
                    .await
                {
                    Ok(Some(account)) => {
                        ...
                        if remaining < 0.000001 {
                            // Very small amount, not worth selling
                            logger.log(
                                "Remaining amount too small, skipping final chunk".to_string(),
                            );
                            break;
                        }
                        remaining
            ...
            let result = match protocol {
                SwapProtocol::PumpFun => {
                    ...
                    match pump
                        .build_swap_from_parsed_data(&trade_info_clone, sell_config.clone())
                        .await
                    {
                        Ok((keypair, instructions, price)) => {
                            logger.log(format!(
                                ...
                                Ok(signatures) => {
                                    ...
                                    .await
                                    {
                                        Ok(verified) => {
                                            if verified {
                                                logger.log(
                                                    "Sell transaction verified successfully"
                                                        .to_string(),
                                                );

                                                Ok(())
                                            } else {
                                                Err("Sell transaction verification failed"
                                                    .to_string())
                                            }
                                        }
                                        Err(e) => {
                ...
                SwapProtocol::Auto | SwapProtocol::Unknown => {
                    logger.log(
                        "Auto/Unknown protocol detected, defaulting to PumpFun for sell"
                            .yellow()
                            .to_string(),
                    );
        ...
        if let Err(e) = cancel_token_monitoring(&token_mint, &logger).await {
            logger.log(
                format!(
                    "Failed to cancel monitoring for token {}: {}",
                    token_mint, e
                )
                .yellow()
                .to_string(),
        ...
        let result = match protocol {
            SwapProtocol::PumpFun => {
                ...
                logger.log(
                    "🔄 Using retry mechanism with Jupiter fallback"
                        .cyan()
                        .to_string(),
                );
                ...
                .await
                {
                    Ok(result) => {
                        if result.success {
                            if result.used_jupiter_fallback {
                                ...
                                logger.log(
                                    format!(
                                        "✅ PumpFun sell succeeded on attempt {}",
                    ...
                    Err(e) => Err(format!("Retry mechanism failed: {}", e)),
                }
            }
            SwapProtocol::PumpSwap => {
                logger.log("Using PumpSwap protocol for sell".to_string());
                    ...
                    Ok(result) => {
                        if result.success {
                            if result.used_jupiter_fallback {
                                ...
                                logger.log(
                                    format!(
                                        "✅ PumpSwap sell succeeded on attempt {}",
                                        result.attempt_count
                                    )
                                    .green()
                                    .to_string(),
                    ...
                    Ok(result) => {
                        if result.success {
                            if result.used_jupiter_fallback {
                                ...
                                logger.log(
                                    format!(
                                        "✅ Raydium sell succeeded on attempt {}",
                                        result.attempt_count
                                    )
                                    .green()
                                    .to_string(),
                    ...
                    Ok(result) => {
                        if result.success {
                            if result.used_jupiter_fallback {
                                ...
                                logger.log(
                                    format!(
                                        "✅ Auto/Unknown sell succeeded on attempt {}",
                                        result.attempt_count
                                    )
                                    .green()
                                    .to_string(),
        ...
        if result.is_ok() {
            ...
            .await
            {
                Ok(cleaned_up) => {
                    if cleaned_up {
                        logger.log(format!(
                            "✅ Comprehensive cleanup completed for standard sell: {}",
                            token_mint
                        ));
                    }
                }
                Err(e) => {
                    logger.log(
                        format!("❌ Error during standard sell cleanup verification: {}", e)
                            .red()
                            .to_string(),
                    );
    ...
    if let Some(_token_info) = BOUGHT_TOKEN_LIST.get(&mint) {
        ...
        tokio::spawn(async move {
            ...
            match selling_engine
                .unified_emergency_sell(&mint_clone, false, None, None)
                .await
            {
                Ok(signature) => {
                    ...
                    if let Some(mut focus_info) = FOCUS_TOKEN_LIST.get_mut(&mint_clone) {
                        ...
                        if !check_and_increment_trade_count(&mint_clone, &logger_clone) {
                            logger_clone.log(
                                format!(
                                    "🚫 Token {} removed due to trade limit after sell",
                                    mint_clone
                                )
                                .red()
                                .to_string(),
                            ...
                            // Cancel price monitoring
                            if let Some((_removed_key, cancel_token)) =
...
async fn handle_volume_based_buying(
    ...
    if let Some(mut focus_info) = FOCUS_TOKEN_LIST.get_mut(&mint) {
        ...
        focus_info
            .slot_price_history
            .push_back((parsed_data.slot, current_price));
        if focus_info.slot_price_history.len() > 16 {
            focus_info.slot_price_history.pop_front();
        }
        ...
        if parsed_data.is_buy
            && parsed_data.sol_change.abs() >= trigger_size
            && focus_info.two_slot_drop_active
        {
            let price_drop_percentage = if focus_info.initial_price > 0.0 {
                ((focus_info.initial_price - focus_info.lowest_price) / focus_info.initial_price)
                    * 100.0
            } else {
                0.0
            };
            ...
            if focus_info.trade_cycles < 3 {
                return execute_sniper_buy(
                    parsed_data,
                    config,
                    focus_info.protocol.clone(),
                    logger,
                )
                .await;
            }
...
async fn execute_sniper_buy(
    ...
    .await
    {
        Ok(_) => {
            ...
            .await
            {
                ...
                Err(e) => {
                    logger.log(
                        format!(
                            "❌ Failed to setup selling strategy for token {}: {}",
                            mint, e
                        )
                        .red()
                        .to_string(),
        ...
        Err(e) => {
            logger.log(
                format!("❌ Sniper buy failed for token {}: {}", mint, e)
                    .red()
                    .to_string(),
            );
            Err(e)
...
async fn handle_parsed_data_for_selling(
                ...
                .any(|target| target == &signer)
            {
                ...
                if let Some(mut token_info) = BOUGHT_TOKEN_LIST.get_mut(&parsed_data.mint) {
                    ...
                    tokio::spawn(async move {
                        ...
                        let config = crate::common::config::Config::get().await;
                        let selling_config =
                            crate::processor::selling_strategy::SellingConfig::set_from_env();
                        let selling_engine = crate::processor::selling_strategy::SellingEngine::new(
                            app_state_clone.clone().into(),
                            Arc::new(config.swap_config.clone()),
                            selling_config,
                        ...
                        match result {
                            Ok(signature) => {
                                ...
                                if let Err(e) =
                                    cancel_token_monitoring(&mint_clone, &logger_clone).await
                                {
                                    logger_clone.log(
                                        format!(
                                            "Failed to cancel monitoring for token {}: {}",
                    ...
                    logger.log(
                        format!(
                            "🎯 Target wallet selling {} but we don't own this token",
                            parsed_data.mint
                        )
                        .blue()
                        .to_string(),
        ...
        if sol_amount >= crate::common::constants::WHALE_SELLING_AMOUNT_FOR_SELLING_TRIGGER {
            ...
            if let Some(mut token_info) = BOUGHT_TOKEN_LIST.get_mut(&parsed_data.mint) {
                ...
                tokio::spawn(async move {
                    ...
                    let result = selling_engine
                        .unified_emergency_sell(&mint_clone, true, None, None)
                        .await;
                    ...
                    match result {
                        ...
                        Err(e) => {
                            logger_clone.log(
                                format!(
                                    "❌ Failed to execute whale emergency sell for token {}: {}",
                                    mint_clone, e
                                )
                                .red()
                                .to_string(),
                ...
                logger.log(
                    format!(
                        "🚀 Whale emergency sell task spawned for token: {}, continuing main flow",
                        parsed_data.mint
                    )
                    .cyan()
                    .to_string(),
                ...
                logger.log(
                    format!(
                        "🐋 Whale selling detected for {} but we don't own this token",
                        parsed_data.mint
                    )
                    .yellow()
                    .to_string(),
    ...
    if let Err(e) = update_token_price_from_trade_info(&mint, &parsed_data).await {
        logger.log(
            format!("Error updating price from transaction: {}", e)
                .yellow()
                .to_string(),
        );
    }
    ...
    if let Err(e) = selling_engine.update_metrics(&mint, &parsed_data).await {
        ...
        logger.log(
            format!("Updated metrics for token: {}", mint)
                .green()
                .to_string(),
        );
    }
    ...
    match selling_engine.evaluate_sell_conditions(&mint).await {
        Ok((should_sell, use_whale_emergency)) => {
            if should_sell {
                logger.log(
                    format!("Sell conditions met for token: {}", mint)
                        .green()
                        .to_string(),
                );
                ...
                if use_whale_emergency {
                    ...
                    logger.log(
                        format!("🐋 WHALE EMERGENCY SELL triggered for token: {}", mint)
                            .cyan()
                            .bold()
                            .to_string(),
                    );
                    ...
                    if let Some(metrics) =
                        crate::processor::selling_strategy::TOKEN_METRICS.get(&mint)
                    {
                        let pnl = if metrics.entry_price > 0.0 {
                            (metrics.current_price - metrics.entry_price) / metrics.entry_price
                                * 100.0
                        } else {
                            0.0
                        };

                        if let Some(_whale_threshold) = selling_engine
                            .get_config()
                            .dynamic_whale_selling
                            .get_whale_threshold_for_pnl(pnl)
                        {
                            ...
                            tokio::spawn(async move {
                                ...
                                match selling_engine_clone
                                    .unified_emergency_sell(
                                        &mint_clone,
                                        true,
                                        Some(&parsed_data_clone),
                                        None,
                                    )
                                    .await
                                {
                                    Ok(signature) => {
                                        ...
                                        // Cancel monitoring task for this token since it's been sold
                                        if let Err(e) =
                                            cancel_token_monitoring(&mint_clone, &logger_clone)
                                                .await
                                        {
                                            logger_clone.log(
                                                format!(
                                    ...
                                    Err(e) => {
                                        logger_clone.log(
                                            format!("🐋 Error executing emergency sell: {}", e)
                                                .red()
                                                .to_string(),
                                        );
                                        ...
                                        logger_clone.log(
                                            "Falling back to regular emergency sell"
                                                .yellow()
                                                .to_string(),
                                        );
                                        match selling_engine_clone
                                ...
                                .await
                            {
                                Ok(_) => {
                                    logger.log(
                                        format!(
                                            "Successfully executed emergency sell for token: {}",
                                            mint
                                        )
                                        .green()
                                        .to_string(),
                    ...
                    logger.log(
                        format!("EMERGENCY SELL ALL triggered for token: {}", mint)
                            .red()
                            .bold()
                            .to_string(),
                    );
                        ...
                        .await
                    {
                        Ok(_) => {
                            logger.log(
                                format!(
                                    "Successfully executed emergency sell all for token: {}",
                                    mint
                                )
                                .green()
                                .to_string(),
                        ...
                        Err(e) => {
                            logger.log(
                                format!("Error executing emergency sell all: {}", e)
                                    .red()
                                    .to_string(),
                            );
                            return Err(format!("Failed to execute emergency sell: {}", e));
                ...
                logger.log(
                    format!("Successfully processed sell for token: {}", mint)
                        .green()
                        .to_string(),
                );
            } else {
                logger.log(
                    format!("Not selling token yet: {}", mint)
                        .blue()
                        .to_string(),
                );
            }
        ...
        Err(e) => {
            logger.log(
                format!("Error evaluating sell conditions: {}", e)
                    .red()
                    .to_string(),
            );
        }
    ...
    logger.log(
        format!(
            "Processing time for sell transaction: {:?}",
            start_time.elapsed()
        )
        .blue()
        .to_string(),
...
async fn setup_selling_strategy(
    ...
    // Initialize
    logger.log(format!(
        "Setting up selling strategy for token: {}",
        token_mint
    ));
    ...
    MONITORING_TASKS.insert(
        token_mint.clone(),
        (token_mint.clone(), cancellation_token.clone()),
    );
...
async fn monitor_token_for_selling(
    ...
    logger.log(
        "Connecting to Yellowstone gRPC for selling, will close connection after selling ..."
            .green()
            .to_string(),
    );
    ...
    // Main stream processing loop
    logger.log(
        "Starting main processing loop with transaction-driven selling checks..."
            .green()
            .to_string(),
    );
    loop {
    ...
    logger.log(
        format!(
            "🔌 gRPC stream and connections properly closed for token: {}",
            token_mint
        )
        .green()
        .to_string(),
        ...
        if !inner_instructions.is_empty() {
            ...
            if let Some(data) = cpi_log_data {
                ...
                tokio::spawn(async move {
                    ...
                    {
                        if parsed_data.mint != "So11111111111111111111111111111111111111112" {
                            let _ = handle_parsed_data_for_selling(
                                parsed_data,
                                config,
                                &txn,
                                target_signature_clone,
                                &logger,
                            )
                            .await;
                        }
...
async fn handle_parsed_data_for_buying(
    ...
    if !parsed_data.is_buy {
        ...
        tokio::spawn(async move {
            ...
            // Use Copy Target Selling mode (higher priority than regular emergency sell)
            let copy_sell_future =
                selling_engine.unified_emergency_sell(&mint_clone, false, None, None);

            match copy_sell_future.await {
                Ok(signature) => {
                    logger_clone.log(
                        format!(
                            "✅ Successfully executed copy sell for token: {} with signature: {}",
                            mint_clone, signature
                        )
                        .green()
                        .to_string(),
                    );
                    ...
                    if let Some(mut token_info) = BOUGHT_TOKEN_LIST.get_mut(&mint_clone) {
                        ...
                        logger_clone.log(
                            format!(
                                "🎯 Copy sell verified - we owned token {} and successfully sold",
                                mint_clone
                            )
                            .green()
                            .to_string(),
                ...
                Err(e) => {
                    logger_clone.log(
                        format!("❌ Copy sell failed for token {}: {}", mint_clone, e)
                            .red()
                            .bold()
                            .to_string(),
                    );
                    ...
                    if let Some(token_info) = BOUGHT_TOKEN_LIST.get(&mint_clone) {
                        logger_clone.log(
                            format!(
                                "🔍 We owned token {} - Amount: {}, Protocol: {:?}",
                                mint_clone, token_info.current_amount, token_info.protocol
                            )
                            .red()
                            .to_string(),
                        );
                        ...
                        logger_clone.log(
                            format!(
                                "🔍 We didn't own token {} - copy sell was unnecessary",
                                mint_clone
                            )
                            .blue()
                            .to_string(),
    ...
    let active_tokens_count = TOKEN_TRACKING.len();
    let active_token_list: Vec<String> = TOKEN_TRACKING
        .iter()
        .map(|entry| entry.key().clone())
        .collect();
    ...
    if active_tokens_count >= config.counter_limit as usize {
        logger.log(
            format!(
                ...
                parsed_data.mint, active_tokens_count, config.counter_limit
            )
            .yellow()
            .to_string(),
        );
    ...
    if parsed_data.is_buy {
        ...
        .await
        {
            ...
            Ok(_) => {
                logger.log(
                    format!(
                        "Processing time for buy transaction: {:?}",
                        start_time.elapsed()
                    )
                    .blue()
                    .to_string(),
                ...
                logger.log(
                    format!(
                        "copied transaction {}",
                        target_signature.clone().unwrap_or_default()
                    )
                    .blue()
                    .to_string(),
                );
                logger.log(
                    format!("Now starting to monitor this token to sell at a profit")
                        .blue()
                        .to_string(),
                );
                ...
                .await
                {
                    ...
                    Err(e) => {
                        logger.log(
                            format!("Failed to set up selling strategy: {}", e)
                                .red()
                                .to_string(),
                        );
                        Err(e)
        ...
        logger.log(
            format!("Not copying selling transaction - using take profit and stop loss")
                .blue()
                .to_string(),
        );
        Ok(())
...
async fn verify_sell_transaction_and_cleanup(
    ...
    logger.log(format!(
        "Starting comprehensive verification and cleanup for token: {}",
        token_mint
    ));
        ...
        match verify_transaction(signature, app_state.clone(), logger).await {
            Ok(verified) => {
                if verified {
                    ...
                    logger.log(
                        format!("❌ Sell transaction verification failed: {}", signature)
                            .red()
                            .to_string(),
                    );
                    return Ok(false);
            ...
            Err(e) => {
                logger.log(
                    format!("❌ Error verifying transaction {}: {}", signature, e)
                        .red()
                        .to_string(),
                );
                return Ok(false);
    ...
    if let Ok(wallet_pubkey) = app_state.wallet.try_pubkey() {
        if let Ok(token_pubkey) = Pubkey::from_str(token_mint) {
                ...
                .await
            {
                Ok(account_result) => {
                    match account_result {
                        Some(account) => {
                            if let Ok(amount_value) = account.token_amount.amount.parse::<f64>() {
                                let decimal_amount =
                                    amount_value / 10f64.powi(account.token_amount.decimals as i32);
                                logger.log(format!(
                                    "Current token balance for {}: {}",
                                    token_mint, decimal_amount
                                ));

                                if decimal_amount <= 0.000001 {
                                    is_fully_sold = true;
                                    logger.log(format!(
                                        "✅ Token {} has zero balance - fully sold",
                                        token_mint
                                    ));
                                } else {
                                    logger.log(format!(
                                        "⚠️  Token {} still has balance: {} - not fully sold",
                                        token_mint, decimal_amount
                                    ));
                                }
                            }
                        ...
                        None => {
                            ...
                            is_fully_sold = true;
                            logger.log(format!(
                                "✅ Token account for {} doesn't exist - fully sold",
                                token_mint
                            ));
                        }
                    }
                ...
                Err(e) => {
                    logger.log(
                        format!("❌ Error checking token balance for {}: {}", token_mint, e)
                            .yellow()
                            .to_string(),
                    );
    ...
    if is_fully_sold {
        ...
        if crate::processor::selling_strategy::TOKEN_TRACKING
            .remove(token_mint)
            .is_some()
        {
        ...
        if crate::processor::selling_strategy::TOKEN_METRICS
            .remove(token_mint)
            .is_some()
        {
        ...
        if let Some((_removed_key, (_token_name, cancel_token))) =
            MONITORING_TASKS.remove(token_mint)
        {
            cancel_token.cancel();
        ...
        logger.log(
            format!("📊 Active tokens count after cleanup: {}", active_count)
                .blue()
                .to_string(),
        );
    }
...
async fn periodic_comprehensive_cleanup(app_state: Arc<AppState>, logger: &Logger) {
    // Get all tokens from both tracking systems
    let tokens_to_check: HashSet<String> = BOUGHT_TOKEN_LIST
        .iter()
        .map(|entry| entry.key().clone())
    ...
    for token_mint in tokens_to_check {
        match verify_sell_transaction_and_cleanup(&token_mint, None, app_state.clone(), logger)
            .await
        {
            Ok(_) => {}
            Err(_) => {}
...
pub async fn trigger_comprehensive_cleanup(
    ...
    use crate::processor::selling_strategy::TokenManager;
    let token_manager = TokenManager::new();
    let selling_strategy_cleaned = match token_manager
        .verify_and_cleanup_sold_tokens(&app_state)
        .await
    {
        Ok(cleaned) => cleaned,
        Err(e) => {
            logger.log(
                format!("Error in selling strategy cleanup: {}", e)
                    .red()
                    .to_string(),
            );
            0
...
pub async fn execute_emergency_sell_via_engine(
    ...
    logger.log(format!(
        "🚀 Executing emergency sell via unified selling engine for token: {}",
        token_mint
    ));
    ...
    match selling_engine
        .unified_emergency_sell(token_mint, false, None, None)
        .await
    {
        Ok(signature) => {
            let elapsed = start_time.elapsed();
            logger.log(
                format!(
                    "⚡ Emergency sell executed in {:?} for token: {} with signature: {}",
                    elapsed, token_mint, signature
                )
                .green()
                .bold()
                .to_string(),
        ...
        Err(e) => {
            logger.log(
                format!("❌ Emergency sell failed for token {}: {}", token_mint, e)
                    .red()
                    .to_string(),
            );
