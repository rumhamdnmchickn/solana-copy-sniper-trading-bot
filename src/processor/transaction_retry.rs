...
pub async fn verify_transaction_with_retry(
    ...
    for attempt in 1..=max_retries {
        if start_time.elapsed() > VERIFICATION_TIMEOUT {
            logger.log(
                format!(
                    "Transaction verification timeout after {:?}",
                    start_time.elapsed()
                )
                .yellow()
                .to_string(),
        ...
        logger.log(format!(
            "Verifying transaction attempt {}/{}: {}",
            attempt, max_retries, signature
        ));
            ...
            .await
        {
                ...
                if let Some(status_opt) = result.value.get(0) {
                    if let Some(status) = status_opt {
                        if status.err.is_none() {
                            logger.log(
                                format!("✅ Transaction verified successfully: {}", signature)
                                    .green()
                                    .to_string(),
                            );
                            return Ok(true);
                            ...
                            logger.log(
                                format!("❌ Transaction failed with error: {:?}", status.err)
                                    .red()
                                    .to_string(),
                            );
                            return Ok(false);
            ...
            Err(e) => {
                logger.log(
                    format!("RPC error during verification attempt {}: {}", attempt, e)
                        .yellow()
                        .to_string(),
                );
            }
    ...
    logger.log(
        format!(
            "Transaction verification failed after {} attempts",
            max_retries
        )
        .red()
        .to_string(),
...
pub async fn execute_sell_with_retry_and_fallback(
    ...
    logger.log(
        format!(
            "🔄 Starting sell transaction with retry for token: {}",
            token_mint
        )
        .cyan()
        .to_string(),
    ...
    match execute_normal_sell_with_retry(trade_info, sell_config.clone(), app_state.clone(), logger)
        .await
    {
        Ok(result) => {
            if result.success {
                logger.log(
                    format!(
                        "✅ Normal sell succeeded on attempt {}",
        ...
        Err(e) => {
            logger.log(
                format!("❌ Normal sell attempts failed: {}", e)
                    .yellow()
                    .to_string(),
            );
        }
    ...
    logger.log(
        format!(
            "🚀 Attempting Jupiter API fallback for token: {}",
            token_mint
        )
        .purple()
        .to_string(),
    ...
    match execute_jupiter_fallback_sell(trade_info, &sell_config, app_state.clone(), logger).await {
        Ok(signature) => {
            logger.log(
                format!("✅ Jupiter fallback sell succeeded: {}", signature)
                    .green()
                    .to_string(),
            );
        ...
        Err(e) => {
            logger.log(
                format!("❌ Jupiter fallback sell failed: {}", e)
                    .red()
                    .to_string(),
            );
            Ok(SellTransactionResult {
...
async fn execute_normal_sell_with_retry(
    ...
    for attempt in 1..=MAX_RETRIES {
        logger.log(
            format!(
                "🔄 Normal sell attempt {}/{} for token: {}",
                attempt, MAX_RETRIES, trade_info.mint
            )
            .cyan()
            .to_string(),
        );

        match execute_single_sell_attempt(
            trade_info,
            sell_config.clone(),
            app_state.clone(),
            logger,
                ...
                match verify_transaction_with_retry(&signature, app_state.clone(), logger, 5).await
                {
                    Ok(verified) => {
                        if verified {
                            ...
                            last_error = format!(
                                "Transaction verification failed for signature: {}",
                                signature
                            );
                            logger.log(
                                format!("❌ Attempt {} failed: {}", attempt, last_error)
                                    .yellow()
                                    .to_string(),
                            );
                        }
        ...
        if attempt < MAX_RETRIES {
            logger.log(
                format!("⏳ Waiting {:?} before retry...", RETRY_DELAY)
                    .yellow()
                    .to_string(),
            );
    ...
    Err(anyhow!(
        "Normal sell failed after {} attempts. Last error: {}",
        MAX_RETRIES,
        last_error
    ))
}
...
async fn execute_pumpfun_sell_attempt(
    ...
    let recent_blockhash =
        crate::library::blockhash_processor::BlockhashProcessor::get_latest_blockhash()
            .await
...
async fn execute_raydium_sell_attempt(
    ...
    let (keypair, instructions, _price) = raydium
        .build_swap_from_parsed_data(trade_info, sell_config)
        .await
...
async fn execute_pumpswap_sell_attempt(
    ...
    let (keypair, instructions, _price) = pump_swap
        .build_swap_from_parsed_data(trade_info, sell_config)
        .await
...
async fn execute_jupiter_fallback_sell(
    ...
    logger.log(
        "🚀 Executing Jupiter API fallback sell"
            .purple()
            .to_string(),
    );
    ...
    let token_amount = token_account
        .token_amount
        .amount
        .parse::<u64>()
    ...
    logger.log(format!(
        "💱 Selling {} tokens via Jupiter API",
        amount_to_sell
    ));
    ...
    let (signature_str, expected_sol) = jupiter_client
        .sell_token(
            &trade_info.mint,
            amount_to_sell,
        ...
        .await
        .map_err(|e| anyhow!("Jupiter API sell failed: {}", e))?;
    ...
    let signature = signature_str
        .parse::<anchor_client::solana_sdk::signature::Signature>()
        .map_err(|e| anyhow!("Failed to parse signature: {}", e))?;

    logger.log(
        format!("✅ Jupiter transaction sent: {}", signature)
            .green()
            .to_string(),
    );
