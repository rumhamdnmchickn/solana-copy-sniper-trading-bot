...
pub struct TradeInfoFromToken {
    ...
    pub is_buy: bool,
    pub price: u64,
    pub _is_reverse_when_pump_swap: bool,
    ...
    if let Some(tx_inner) = &txn.transaction {
        if let Some(meta) = &tx_inner.meta {
            ...
            return meta
                .log_messages
                .iter()
                .any(|log| log.contains("Program log: Instruction: MintTo"));
        }
    }
    false
}
...
pub fn parse_transaction_data(
    ...
    match buffer.len() {
        368 => {
            // pump swap transaction - 368 bytes
            ...
            let (price, _is_reverse_when_pump_swap) =
            ...
            let is_buy = if _is_reverse_when_pump_swap {
            ...
            let (sol_change, token_change) = if _is_reverse_when_pump_swap {
                // Reverse case: base_mint is WSOL, quote_mint is token
                if is_buy {
                    // Buy: spend SOL (base), get tokens (quote)
                    (
                        -(base_amount_in_or_base_amount_out as f64) / 1_000_000_000.0,
            ...
            let liquidity = if !_is_reverse_when_pump_swap {
        ...
        270 => {
            // pump swap migeration transaction - 270 bytes
            // Extract token mint and check for reverse case
            let mint = extract_token_info(&txn);
            let timestamp = parse_u64(buffer, 16)?;
            let base_amount_in_or_base_amount_out = parse_u64(buffer, 24)?;
            ...
            let _is_reverse_when_pump_swap = mint == "So11111111111111111111111111111111111111112";
            ...
            let price = if pool_base_token_reserves > 0 && pool_quote_token_reserves > 0 {
                if _is_reverse_when_pump_swap {
                    // In reverse case: poolBaseTokenReserves/poolQuoteTokenReserves (base_mint is WSOL)
                    pool_base_token_reserves.saturating_mul(1_000_000_000)
                        / pool_quote_token_reserves.max(1)
                } else {
                    // Normal case: poolQuoteTokenReserves/poolBaseTokenReserves (quote_mint is WSOL)
                    pool_quote_token_reserves.saturating_mul(1_000_000_000)
                        / pool_base_token_reserves.max(1)
                }
            } else {
                0
        ...
        266 => {
            ...
            Some(TradeInfoFromToken {
                ...
                is_buy,
                price,
                _is_reverse_when_pump_swap: false, // PumpFun is never reverse
        ...
        146 => {
            // Raydium Launchpad - process all buy transactions
            ...
            let sol_change_amount =
                (real_quote_after as i64 - real_quote_before as i64) as f64 / 1_000_000_000.0;
            let token_change_amount =
                (real_base_after as i64 - real_base_before as i64) as f64 / 1_000_000_000.0;
            ...
            let price = if (virtual_base_reserve as f64 - real_base_after as f64) > 0.0 {
                let calculated_price = ((virtual_quote_reserve as f64 + real_quote_after as f64)
                    / (virtual_base_reserve as f64 - real_base_after as f64))
                    * 1_000_000_000.0; // never change this formula without Deni's permission
                ...
                dex_log(
                    "Raydium Launchpad: Price calculation failed - division by zero"
                        .red()
                        .to_string(),
                );
                0u64
            ...
            Some(TradeInfoFromToken {
                ...
                is_buy,
                price,
                _is_reverse_when_pump_swap: false, // Raydium is never reverse
