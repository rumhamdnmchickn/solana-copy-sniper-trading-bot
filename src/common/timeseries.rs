...
impl TokenTimeseries {
    ...
    pub fn update(&mut self, slot: u64, price: f64, is_buy: bool, sol_volume: f64) {
        ...
        if let Some(back) = self.samples.back_mut() {
            if back.slot == slot {
                back.price = price;
                if is_buy {
                    back.buy_volume += sol_volume;
                } else {
                    back.sell_volume += sol_volume;
                }
                return;
            }
        }
        ...
        if is_buy {
            sample.buy_volume = sol_volume;
        } else {
            sample.sell_volume = sol_volume;
        }
    ...
    pub fn detect_bottom_after_drop(
        &self,
        min_drop_pct: f64,
        sell_decline_pct: f64,
        stabilize_slots: usize,
    ) -> BottomSignal {
        if self.samples.len() < stabilize_slots * 2 + 2 {
            return BottomSignal::no();
        }

        let high = match self.highest_price() {
            Some(h) => h,
            None => return BottomSignal::no(),
        };
        let low = match self.lowest_price() {
            Some(l) => l,
            None => return BottomSignal::no(),
        };
        if high <= 0.0 {
        ...
        let recent: Vec<&SlotSample> = self
            .samples
            .iter()
            .skip(n - stabilize_slots)
            .take(stabilize_slots)
            .collect();
        let prev: Vec<&SlotSample> = self
            .samples
            .iter()
        ...
        for w in recent.as_slice().windows(2) {
            if w[1].price + (w[1].price * 0.001) < w[0].price {
                // 0.1% tolerance
        ...
        let recent_sell_avg =
            recent.iter().map(|s| s.sell_volume).sum::<f64>() / stabilize_slots as f64;
        let prev_sell_avg =
            prev.iter().map(|s| s.sell_volume).sum::<f64>() / stabilize_slots as f64;
        if prev_sell_avg <= 0.0 {
            return BottomSignal::no();
        }
        let decline_pct = if prev_sell_avg > 0.0 {
            (prev_sell_avg - recent_sell_avg) / prev_sell_avg * 100.0
        } else {
            0.0
        };
        if decline_pct < sell_decline_pct {
            return BottomSignal::no();
        }
...
pub fn update_for_mint(mint: &str, slot: u64, price: f64, is_buy: bool, sol_volume: f64) {
    let mut entry = TOKEN_TIMESERIES
        .entry(mint.to_string())
        .or_insert_with(|| TokenTimeseries::new(20));
    entry.update(slot, price, is_buy, sol_volume);
}

pub fn analyze_bottom(
    mint: &str,
    min_drop_pct: f64,
    sell_decline_pct: f64,
    stabilize_slots: usize,
